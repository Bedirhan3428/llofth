...
export default function Sidebar() {
  ...
  return (
    <>
      {/* 1. MASAÜSTÜ & SMART TV: Sol Kenar Çubuğu (md:flex) */}
      <aside
        onMouseEnter={() => setIsExpanded(true)}
        onMouseLeave={() => setIsExpanded(false)}
        onFocusCapture={() => setIsExpanded(true)}
        onBlurCapture={() => setIsExpanded(false)}
        className={`hidden md:flex fixed top-0 left-0 h-screen z-40 transition-all duration-200 ease-out bg-[#0d0e14]/95 backdrop-blur-md border-r border-white/5 flex-col justify-between py-5 px-2.5 ${
          isExpanded ? 'w-52 shadow-2xl' : 'w-16'
        }`}
      >
        <div>
          {/* llofth Logo Link */}
